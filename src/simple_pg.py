...
def circle(
    ...
    if filled:
        if antialias:
            ...
            # surface.blit(s, (x, y))
            if r > 1:
                gfxdraw.filled_circle(surface, int(x), int(y), int(r - 1), color)
                gfxdraw.aacircle(surface, int(x), int(y), int(r - 1), color)
            gfxdraw.aacircle(surface, int(x), int(y), int(r), color)
        else:
            gfxdraw.filled_circle(surface, int(x), int(y), int(r), color)
    else:
        gfxdraw.aacircle(surface, int(x), int(y), int(r), color)
...
def image(img, x=0, y=0, surface: pygame.Surface = None):
    """
...
def go(
    ...
    import inspect
    # Get draw function from back frame
    try:
        draw_func = inspect.currentframe().f_back.f_locals["draw"]
    except KeyError:
        draw_func = inspect.currentframe().f_back.f_globals["draw"]
