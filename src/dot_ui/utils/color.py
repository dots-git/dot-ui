...
class Color:
    ...
    def __init__(
        ...
        if not isinstance(color_format, str):
            color_format = "rgb"
        convert_from_float = color_format.endswith("f") or color_format.endswith(
            "float"
        )
        if isinstance(v1, str):
            ...
            raise ValueError(
                "Use hex string or iterable as first argument or give at least values 1 through 3"
            )
        if v4 is None:
        ...
        for v in (v1, v2, v3, v4):
            if not isinstance(v, (int, float)):
                raise ValueError(
                    "Unsupported type "
                    + type(v).__name__
                    + ". Use int, float or hex string representation"
                )
    ...
    def as_hls(self, include_alpha=True):
        h, l, s = colorsys.rgb_to_hls(self.r / 255.0, self.g / 255.0, self.b / 255)
        if include_alpha:
