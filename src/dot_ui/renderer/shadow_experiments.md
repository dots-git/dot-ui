Old renderer that includes shadows. They were pretty slow and didn't look good for text in particular so I removed them.
...
class Renderer:
    ...
    # 0: No shadow, 
    # 1: shadow around entire text body, 
    # 2: shadow around each word (recommended), 
    # 3: shadow around each character
    ...
    @staticmethod
    def update():
        Renderer._shadow_slices = {}
        Renderer._shadow_surfaces = {}

    @staticmethod
    def set_default_color(color):
        Renderer._color = color
    ...
    def set_corner_radius(radius, update=False):
        ...
        Renderer._corner_radius = radius
        if update:
            Renderer.update()
    ...
    def set_shadow_radius(radius, update=False):
        ...
        Renderer._shadow_radius = radius
        if update:
            Renderer.update()
    ...
    def set_shadow_offset(offset: Vector2):
        ...
        Renderer._shadow_offset = offset

    @staticmethod
    def set_shadow_opacity(opacity: float):
        """Change the shadow opacity"""
        Renderer._shadow_opacity = opacity
    ...
    def _precompute_shadow_slices(pseudo_radius):
        ...
        cutoff_at = 0.5 / 255
        surface_size = int(
            sqrt(-log(cutoff_at)) * Renderer._shadow_radius
            + pseudo_radius
            - Renderer._shadow_radius
        )
        ...
        for x in range(surface_size):
            for y in range(surface_size):
                ...
                actual_distance = distance - pseudo_radius + Renderer._shadow_radius
                if actual_distance < 0:
                    alpha = 255
                else:
                    alpha = (
                        exp(-((actual_distance / Renderer._shadow_radius) ** 2))
                        * 255
                    )
                    mod = alpha % 1
                    alpha = int(alpha) + (
        ...
        for x in range(surface_size):
            ...
            if actual_distance < 0:
                ...
                alpha = (
                    exp(-((actual_distance / Renderer._shadow_radius) ** 2)) * 255
                )
                mod = alpha % 1
        ...
        right_block = pygame.Surface(
            (surface_size, Renderer._slice_block_size), pygame.SRCALPHA
        )
        for x in range(surface_size):
            for y in range(Renderer._slice_block_size):
                distance = x
                actual_distance = distance - pseudo_radius + Renderer._shadow_radius
                if actual_distance < 0:
                    alpha = 255
        ...
        Renderer._shadow_slice_top = top
        Renderer._shadow_slice_bottom = bottom
        Renderer._shadow_slice_left = left
        Renderer._shadow_slice_right = right

        Renderer._shadow_slice_top_block = top_block
        Renderer._shadow_slice_bottom_block = bottom_block
        Renderer._shadow_slice_left_block = left_block
        Renderer._shadow_slice_right_block = right_block

        Renderer._shadow_slice_top_left = top_left
        Renderer._shadow_slice_top_right = top_right
        Renderer._shadow_slice_bottom_left = bottom_left
        Renderer._shadow_slice_bottom_right = bottom_right

        Renderer._shadow_slices[pseudo_radius] = {
            "used": True,
            "top": top,
            "bottom": bottom,
            "left": left,
    ...
    def _get_shadow(
        ...
        if radius is None:
            radius = Renderer._corner_radius
        ...
        pseudo_radius = max(radius, Renderer._shadow_radius)

        shadow_opacity = min_size / 2 / pseudo_radius
        shadow_opacity = 1 if shadow_opacity >= 1 else shadow_opacity

        if not pseudo_radius in Renderer._shadow_slices.keys():
            Renderer._precompute_shadow_slices(pseudo_radius)
        Renderer._shadow_slices[pseudo_radius]["used"] = True

        slice_size = Renderer._shadow_slices[pseudo_radius][
            "bottom_right"
        ].get_size()[0]

        radius_offset = min(
            max(Renderer._shadow_radius - radius, radius),
            int(min_size / 2),
        )
        ...
        if surface_key in Renderer._shadow_surfaces.keys():
            Renderer._shadow_surfaces[surface_key]["used"] = True
            return (
                Renderer._shadow_surfaces[surface_key]["surface"],
                Renderer._shadow_offset.x + x - slice_size + radius_offset,
                Renderer._shadow_offset.y + y - slice_size + radius_offset,
            )
        print(
            f"Assembling shadow surface {surface_key, Renderer._shadow_surfaces.keys()}"
        )

        keys = [key for key in Renderer._shadow_slices[pseudo_radius].keys()]

        for key in keys:
            if key != "used":
                Renderer._shadow_slices[pseudo_radius][key].set_alpha(
        ...
        shadow_surface.blit(
            Renderer._shadow_slices[pseudo_radius]["top_left"],
            (
                0,
                0,
            ),
        ...
        shadow_surface.blit(
            Renderer._shadow_slices[pseudo_radius]["bottom_right"],
            (
        ...
        shadow_surface.blit(
            Renderer._shadow_slices[pseudo_radius]["top_right"],
            (
                width - 2 * radius_offset + slice_size,
                0,
            ),
        ...
        shadow_surface.blit(
            Renderer._shadow_slices[pseudo_radius]["bottom_left"],
            (
                0,
                height - 2 * radius_offset + slice_size,
            ),
        ...
        x_block_amt = int(x_range_to_fill / Renderer._slice_block_size)
        x_block_end = x_block_amt * Renderer._slice_block_size

        y_range_to_fill = height - 2 * radius_offset
        y_block_amt = int(y_range_to_fill / Renderer._slice_block_size)
        y_block_end = y_block_amt * Renderer._slice_block_size

        top_block = Renderer._shadow_slices[pseudo_radius]["top_block"]
        bottom_block = Renderer._shadow_slices[pseudo_radius]["bottom_block"]

        for i in range(x_block_amt):
            shadow_surface.blit(
                top_block,
                (
                    i * Renderer._slice_block_size + slice_size,
                    0,
                ),
            )
            shadow_surface.blit(
        ...
        left_block = Renderer._shadow_slices[pseudo_radius]["left_block"]
        right_block = Renderer._shadow_slices[pseudo_radius]["right_block"]
        ...
        top = Renderer._shadow_slices[pseudo_radius]["top"]
        bottom = Renderer._shadow_slices[pseudo_radius]["bottom"]
        ...
        left = Renderer._shadow_slices[pseudo_radius]["left"]
        right = Renderer._shadow_slices[pseudo_radius]["right"]
        ...
        Renderer._shadow_surfaces[surface_key] = {
    ...
    @staticmethod
    def tick():
        keys = [key for key in Renderer._shadow_slices.keys()]
        for key in keys:
            if Renderer._shadow_slices[key]["used"] != True:
                Renderer._shadow_slices.pop(key)
            else:
                Renderer._shadow_slices[key]["used"] = False
        keys = [key for key in Renderer._shadow_surfaces.keys()]
        for key in keys:
            if Renderer._shadow_surfaces[key]["used"] != True:
                Renderer._shadow_surfaces.pop(key)
            else:
                Renderer._shadow_surfaces[key]["used"] = False
    ...
    def render(widget: Widget, delta: float):
        ...
        if isinstance(widget, Container):
            ...
            for child in widget.child_list:
                Renderer.render(child, delta)
                for surface, x, y in child.shadow_surfaces:
                    widget.surface.blit(surface, (x, y))

            shadow_surface.set_alpha(Renderer._shadow_opacity * 255)
            ...
            for child in widget.child_list:
                ...
                widget.surface.blit(
                    rounded(child.surface, Renderer._corner_radius),
            ...
            if Renderer._text_shadow_detail == 1:
            ...
            if Renderer._text_shadow_detail == 3:
                char_pos = 0
                for char in metrics:
                    widget.shadow_surfaces.append(
                        Renderer._get_shadow(
